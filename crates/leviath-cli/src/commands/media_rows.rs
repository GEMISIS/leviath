//! Editing the rows in `media_types.toml` from the command line.
//!
//! `lev media add` and `lev media remove` change one row at a time in the
...
/// What `lev media add` sets on a row. Every field optional: a row names
...
/// The table the rows live in: a `[media_types]` wrapper when the file
...
fn check_loads(doc: &DocumentMut, path: &Path) -> Result<(), String> {
    ...
    registry
        .layer(&rows, crate::config::MEDIA_TYPES_FILE)
...
pub(crate) fn remove_row(path: &Path, key: &str) -> Result<(), String> {
    ...
    if rows.remove(key).is_none() {
        return Err(format!(
            "no row for {key} in {}; `lev media list` shows every row and where it lives",
            path.display()
        ));
    }
    write_doc(&doc, path)
...
mod tests {
    ...
    fn rows_are_added_updated_and_removed_in_place() {
        ...
        let path = dir.path().join("nested").join("media_types.toml");
        let edit = RowEdit {
    ...
    /// A file carrying a `[media_types]` wrapper is edited inside it, so the
    /// block keeps loading the way the config reads it.
    #[test]
    fn a_wrapped_file_is_edited_inside_the_wrapper() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("media_types.toml");
        std::fs::write(&path, "[media_types.\"x/y\"]\nfamily = \"a\"\n").unwrap();
        let edit = RowEdit {
    ...
    fn an_edit_that_would_not_load_is_refused_before_it_is_written() {
        ...
        assert!(bad_key.contains("media_types key png"), "{bad_key}");

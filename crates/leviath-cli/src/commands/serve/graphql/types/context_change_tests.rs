    ...
    crate::runstate::with_isolated_runs_dir_async("graphql-context-changes", |_dir| async move {
        ...
        let json = data(
            r#"{ run { contextChanges(first: 10) {
                 ...
                 edges { cursor node {
                   region cause entriesAdded entriesRemoved tokenDelta at
                 } }
               } } }"#,
        )
        .await;
        ...
        assert_eq!(first["node"]["region"], "plan");
        assert_eq!(first["node"]["cause"], "SEED");
        assert_eq!(first["node"]["entriesAdded"], 1);
        assert_eq!(first["node"]["entriesRemoved"], 0);
        assert_eq!(first["node"]["tokenDelta"], 40);
        assert_eq!(first["node"]["at"], 20);

        let second = &page["edges"][1]["node"];
        assert_eq!(second["cause"], "COMPACTION");
        assert_eq!(second["entriesRemoved"], 3);
        assert_eq!(second["tokenDelta"], -120);
    })
    .await;
}
...
async fn a_run_with_no_recorded_causes_has_no_changes() {
    crate::runstate::with_isolated_runs_dir_async(
        ...
        |_dir| async move {
            create_run(&meta()).expect("run written");
            let json =
                data("{ run { contextChanges(first: 10) { total edges { node { region } } } } }")
                    .await;

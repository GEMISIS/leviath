...
impl RunMutation {
    ...
    ///
    /// Format only: nothing is dialled and nothing is written, which is what makes
    /// it safe to run on every keystroke of a form. It is a mutation because it
    /// belongs beside the config write it precedes, not because it changes
    /// anything. `checkProvider` is the one that asks the account.
...
fn field_of(decided: &serde_json::Value, key: &str) -> String {
    decided
        .get(key)
        .and_then(|value| value.as_str())
        .unwrap_or_default()

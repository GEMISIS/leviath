...
mod tests {
    ...
    fn a_stat_cache_honours_the_recheck_window() {
        ...
        // the point is not to stat; outside it the change is seen.
        std::fs::write(&path, "2").unwrap();
        let hour = std::time::Duration::from_secs(3600);
        assert_eq!(*cache.get_with_recheck(&path, parse, hour).unwrap(), "1");
        assert_eq!(*cache.get_with(&path, parse).unwrap(), "2");
        ...
        assert_eq!(*cache.get_with(&path, parse).unwrap(), "2");
        assert_eq!(*cache.get_with_recheck(&path, parse, hour).unwrap(), "2");

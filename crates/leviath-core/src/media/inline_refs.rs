...
pub fn extract(text: &str, resolve: &mut dyn FnMut(&str) -> bool) -> Extracted {
    ...
    while i < chars.len() {
                ...
                .is_some_and(|n| !n.is_whitespace() && *n != '@')
        {
            ...
            let (path, media_type) = split_type(trimmed);
...
/// is not followed by a media type stays part of the path (Windows drives,
/// `file:` URLs, a stray colon).
fn split_type(raw: &str) -> (String, Option<MediaType>) {
    if let Some((path, suffix)) = raw.rsplit_once(':')
        && suffix.contains('/')
        && let Ok(t) = MediaType::parse(suffix)
    {
...
mod tests {
    ...
    fn type_suffix_is_split_off() {
        ...
        assert!(e.refs[0].media_type.is_none());
    }

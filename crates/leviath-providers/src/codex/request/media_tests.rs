...
fn png(name: &str) -> ContentBlock {
    ...
    if let ContentBlock::Media { data, .. } = &mut block {
        *data = "AQID".to_string();
    }
    block
}
...
#[test]
fn text_and_media_share_one_message_item() {
...
#[test]
fn media_alone_makes_an_item_with_no_empty_text_part() {
    let mut input = Vec::new();
...
#[test]
fn media_before_a_tool_call_is_flushed_ahead_of_it() {
    let mut input = Vec::new();
    push_message(
        &mut input,
        &message(vec![

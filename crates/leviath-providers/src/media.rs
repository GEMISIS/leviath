...
//! them in.
//!
//! Assembly never reads a file: it emits a [`ContentBlock::Media`] per stored
...
//!
//! - the bytes, base64, when the model's [`ModelMedia`] covers the type;
...
use base64::Engine;
use leviath_core::media::{BlobRef, Delivery, MediaRegistry, MediaType};
...
pub fn family_of(media_type: &MediaType) -> Family {
    match (media_type.kind(), media_type.subtype()) {
...
/// `data:<type>;base64,<bytes>` for a hydrated media block.
pub fn data_uri(media_type: &MediaType, data: &str) -> String {
...
pub struct Hydration<'a> {
    ...
    pub media: &'a ModelMedia,
    /// The registry, for the text flag.
    pub registry: &'a MediaRegistry,
    /// The most media blocks kept with bytes; the oldest beyond it become
    /// stand-ins.
...
fn fate(block: &ContentBlock, h: &Hydration<'_>) -> Fate {
    let ContentBlock::Media { part, deliver, .. } = block else {
        return Fate::StandIn;
    };
    match deliver {
        ...
        Some(Delivery::Native) | None => {
            if h.media.accepts(&part.media_type) {
                Fate::Bytes
            } else if h.registry.info(&part.media_type).text {
                Fate::Text
            } else {
                Fate::StandIn
            }
...
/// Fill every media block in `request` with what the model should get.
...
/// byte heuristic, a media block by its registry estimate whether or not it
/// carries bytes yet.
pub fn block_tokens(block: &ContentBlock) -> usize {
    match block {
        ...
        ContentBlock::Media { part, .. } => part.tokens,
    }
}

/// The tokens the media blocks of `request` are estimated to cost, from
/// their registry estimates. The text guard counts only text; this is the
/// rest.
pub fn media_tokens(request: &InferenceRequest) -> usize {
    request
        .messages
        .iter()
        ...
        .filter_map(|b| match b {
            ContentBlock::Media { part, data, .. } if !data.is_empty() => Some(part.tokens),
            _ => None,
        })
        .sum()
}

/// A media block as OpenAI's Chat Completions content part, or a text part
...
pub fn openai_part(block: &ContentBlock) -> Option<serde_json::Value> {
    ...
    Some(match family_of(&part.media_type) {
        Family::Image => serde_json::json!({
            "type": "image_url",
            "image_url": { "url": data_uri(&part.media_type, data) },
        }),
        Family::Audio => serde_json::json!({
            "type": "input_audio",
            "input_audio": { "data": data, "format": audio_format(&part.media_type) },
        }),
        Family::Document => serde_json::json!({
            "type": "file",
            "file": {
                "filename": name.clone().unwrap_or_else(|| "document.pdf".to_string()),
                "file_data": data_uri(&part.media_type, data),
            },
        }),
...
/// A media block as an Anthropic content block: `image` or `document`, or a
...
pub fn anthropic_block(block: &ContentBlock) -> Option<serde_json::Value> {
    let ContentBlock::Media { part, data, .. } = block else {
        return None;
    };
    if data.is_empty() {
        return Some(serde_json::json!({ "type": "text", "text": part.stand_in }));
...
/// A media block as a Codex Responses content part: `input_image`,
...
pub fn codex_part(block: &ContentBlock) -> Option<serde_json::Value> {
    ...
    Some(match family_of(&part.media_type) {
        Family::Image => serde_json::json!({
            "type": "input_image",
            "image_url": data_uri(&part.media_type, data),
        }),
        Family::Document => serde_json::json!({
            "type": "input_file",
            "filename": name.clone().unwrap_or_else(|| "document.pdf".to_string()),
...
fn audio_format(media_type: &MediaType) -> &'static str {
...
mod tests {
    ...
    use leviath_core::media::{Blob, Part};

    fn reg() -> MediaRegistry {
    ...
    fn obj_part() -> Part {
        let blob = Blob::new(
            MediaType::parse("model/obj").unwrap(),
            b"v 1 2 3\n".to_vec(),
        )
        .named("cube.obj");
    ...
    fn every_block_kind_has_a_token_share() {
        ...
        assert_eq!(
            block_tokens(&ContentBlock::media(&png_part()).unwrap()),
            1600
        );
    }
    ...
    fn families_and_uris() {
        assert_eq!(
            family_of(&MediaType::parse("image/webp").unwrap()),
            Family::Image
        );
        assert_eq!(
            family_of(&MediaType::parse("audio/wav").unwrap()),
            Family::Audio
        );
        assert_eq!(
            family_of(&MediaType::parse("video/mp4").unwrap()),
            Family::Video
        );
        assert_eq!(
            family_of(&MediaType::parse("application/pdf").unwrap()),
            Family::Document
        );
        assert_eq!(
            family_of(&MediaType::parse("model/obj").unwrap()),
            Family::Other
        );
        assert_eq!(
            data_uri(&MediaType::parse("image/png").unwrap(), "AAAA"),
            "data:image/png;base64,AAAA"
        );
        assert_eq!(
            audio_format(&MediaType::parse("audio/mpeg").unwrap()),
            "mp3"
        );
        assert_eq!(audio_format(&MediaType::parse("audio/wav").unwrap()), "wav");
    }

    #[test]
    fn a_vision_model_gets_the_bytes_and_a_text_model_the_stand_in() {
        let parts = [png_part()];
        let block = ContentBlock::media(&parts[0]).unwrap();
        assert!(!block.is_hydrated_media());
        ...
        assert!(blocks[1].is_hydrated_media());
        assert_eq!(media_tokens(&req), 1600);
        assert!(blocks[1].stand_in().is_some());
        assert_eq!(ContentBlock::Text { text: "x".into() }.stand_in(), None);

        let text_only = ModelMedia::text_only();
    ...
    fn the_text_bypass_and_the_delivery_overrides() {
        ...
        let fetch = bytes_of(&parts);
        let text_only = ModelMedia::text_only();
        let obj = ContentBlock::media(&parts[0]).unwrap();
        ...
        let forced = ContentBlock::media(&parts[1].clone().delivered(Delivery::Text)).unwrap();
        ...
        let vision = ModelMedia::new(&["text/*", "image/*"], &["text/*"]);
        let quiet = ContentBlock::media(&parts[1].clone().delivered(Delivery::StandIn)).unwrap();
        ...
        let native = ContentBlock::media(&parts[1].clone().delivered(Delivery::Native)).unwrap();
    ...
    fn missing_bytes_and_the_cap_degrade_to_stand_ins() {
        ...
        let vision = ModelMedia::new(&["text/*", "image/*"], &["text/*"]);
        let mut req = request(vec![
            ContentBlock::media(&parts[0]).unwrap(),
            ContentBlock::media(&parts[1]).unwrap(),
        ]);
        let report = hydrate_request(
            &mut req,
            &Hydration {
        ...
        assert_eq!(report.sent, 1);
        let blocks = blocks_of(&req.messages[1].content);
        assert!(!blocks[0].is_hydrated_media() && blocks[2].is_hydrated_media());
    }

    #[test]
    fn vendor_shapes() {
        let parts = [png_part(), obj_part()];
        let png = ContentBlock::media(&parts[0]).unwrap();
        ...
        let make = |t: &str, name: &str| {
            let blob = Blob::new(MediaType::parse(t).unwrap(), vec![1, 2, 3]).named(name);
            let part = Part::stored(blob.describe(&reg())).named(name);
            with_data(ContentBlock::media(&part).unwrap(), "AQID")
        };
        ...
        let unnamed_pdf = {
            let blob = Blob::new(MediaType::parse("application/pdf").unwrap(), vec![1]);
            with_data(
                ContentBlock::media(&Part::stored(blob.describe(&reg()))).unwrap(),
                "AQ==",
            )
        };
        assert_eq!(
        ...
        assert!(ContentBlock::media(&Part::text("t")).is_none());
    }
}

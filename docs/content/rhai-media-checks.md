---
title: Rhai media checks
description: Say what bytes must look like to be stored as one of your media types, so a mislabelled file is refused where it arrives, not three tools later.
group: Reference
group_order: 3
order: 13
---
...
A [media type](/docs/media) is a claim. Leviath checks the claim's spelling wherever a type is
...
```toml
# ~/.leviath/media_types.toml, or [media_types] in a blueprint
...
[`media_types.toml`](/docs/configuration#media_typestoml) and a `[media_types]` table in
`config.toml`, the blueprint's own directory for a row in a manifest. It has to resolve inside
that directory, the same fence every other script gets. `lev media add <type> --check <path>`
...
`lev media list` shows the check each type answers to, and `lev media check <file>` runs it and
prints the verdict.
...
`lev media`, and the daemon keeps the compiled defaults rather than a registry with a check it
cannot run.
